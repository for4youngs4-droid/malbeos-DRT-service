...
- 색은 브랜드 그라데이션 기준: 민트 #8decd0 → 블루 #207fba (토큰은 `app/globals.css`)
...
- `/onboarding` 온보딩 4장
- `/rider` 홈 (하단 탭: 홈 / 이동계획 / 함께이동 / 내 루틴 / 설정. 음성 예약은 홈에서 큰 마이크로 바로 한다)
- `/rider/voice` 음성 예약
- `/rider/routine-alert` 루틴 알림
- `/rider/routines` 내 루틴
- `/rider/chain` 하루 이동 계획
- `/rider/live` 이동 중
- `/rider/together` 함께 이동 + 운행 현황
- `/admin` 묶기 전후 비교 한 화면
- 기사 앱은 만들지 않는다
