...
- 색은 브랜드 그라데이션 하나로 통일: 민트 #8decd0 → 블루 #207fba. 채워진 요소(마이크, 버튼, 선택된 탭)는 이 그라데이션. 스위치 바탕은 플랫 단색(켜짐 #207fba, 꺼짐 회청색), 카드·목록 아이콘 칩은 연한 회색 플랫. 예외 색은 알림 빨강, 준비 중 주황, 예약됨 초록 세 가지뿐 (정의는 `app/globals.css`, `lib/colors.ts`)
