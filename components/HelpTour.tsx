...
type Display = { stepIdx: number; rect: Rect | null };
...
export default function HelpTour() {
  ...
  // 설명 글과 하이라이트는 함께 움직여야 자연스럽다. 그래서 다음 자리를 다 잰 뒤에만
  // 한 번에 바꿔 보여주고, 그 전까지는 지금 보이는 자리를 그대로 둔다(글만 먼저 튀지 않게)
  const [display, setDisplay] = useState<Display | null>(null);
  const tries = useRef(0);
  const scrolled = useRef(false);

  const target = step !== null && step < TOUR_STEPS.length ? TOUR_STEPS[step] : null;

  // 화면이 바뀌는 단계면 그 화면으로 실제로 들어간다 (같은 화면 안의 다음 부분이면 이동하지 않는다)
  useEffect(() => {
    if (target && pathname !== target.href) router.push(target.href);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [step]);

  // 그 부분이 화면에 나타날 때까지 잠깐씩 다시 재본다 (페이지 전환 애니메이션 시간만큼).
  // 화면이 밀려 들어오는 애니메이션(320ms)이 끝나기 전에 재면 자리가 살짝 어긋난 채로 굳어버리므로,
  // 값이 두 번 연속 똑같이 나올 때까지는 확정하지 않는다. 확정되기 전까지는 지금 보이는
  // 설명 글·하이라이트를 그대로 둬서, 글만 먼저 바뀌고 하이라이트가 뒤늦게 따라오지 않게 한다.
  ...
  useEffect(() => {
    ...
    const attempt = () => {
      ...
      const el = document.querySelector(`[data-tour-target="${target.target}"]`);
      if (host && el) {
        ...
        last = measured;
        if (same) {
          setDisplay({ stepIdx: step, rect: measured }); // 설명 글과 하이라이트를 한 번에 바꾼다
          return;
        }
      ...
      tries.current += 1;
      if (tries.current > 40) {
        if (target.optional) nextTourStep();
  ...
  useEffect(() => {
    ...
    const step2 = TOUR_STEPS[display.stepIdx];
    const update = () => {
      const host = document.getElementById("phone-overlay");
      const el = document.querySelector(`[data-tour-target="${step2.target}"]`);
      if (!host || !el) return;
      const h = host.getBoundingClientRect();
      const t = el.getBoundingClientRect();
      setDisplay({ stepIdx: display.stepIdx, rect: { top: t.top - h.top, left: t.left - h.left, width: t.width, height: t.height, hostH: h.height } });
    };
  ...
  const isLast = display.stepIdx === TOUR_STEPS.length - 1;
  const progress = ((display.stepIdx + 1) / TOUR_STEPS.length) * 100;
      ...
      <div key={display.stepIdx} className="page-in-up pointer-events-auto absolute inset-x-6" style={captionStyle}>
        <div className="flex items-center justify-between">
          <span className="flex items-center gap-2 rounded-pill bg-white/15 py-1 pl-1.5 pr-3 text-[13px] font-semibold text-white backdrop-blur-sm">
            ...
            {current.section} · {display.stepIdx + 1}/{TOUR_STEPS.length}
          </span>
